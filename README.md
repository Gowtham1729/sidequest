...
Every game keeps its tailored synthesized event sounds. Thirty-two games play short recorded music loops (AI-generated, stored in `dist/audio/assets/` and attached via `dist/audio/recorded.js`) the first time they are played; Echo and Beat Drop deliberately leave background music silent so memory cues and rhythm timing stay clear. Game visuals and gameplay use the existing centralized audio lifecycle. Music plays only during an active round, stops for pause, feed gestures, dialogs, and backgrounding, and starts a fresh phrase on resume. Switching games stops the previous game's voices. Results have short ending cues without continuing the music. Apart from those loops, no sound files, downloads, libraries, or third-party music services are required.

- `dist/audio/engine.js`: one lazy Web Audio context, separate music/effects gains, output limiter, short look-ahead music scheduler, bounded voices, a bounded decoded-buffer cache, device preferences, and lifecycle cleanup.
- `dist/audio/profiles.js`: declarative sound palettes and original synthesized music; `dist/audio/expansion-profiles.js` supplies the 15 additions; `dist/audio/recorded.js` overrides music with recorded loops where available.
...
Files are fetched and decoded lazily, cached per session, and played through the same controls. The decoded-buffer cache is bounded, so long feed sessions only keep the most recent loops in memory; evicted loops refetch on demand. Loading/decoding failures are silent; late loads cannot leak across a game switch, pause, or mute. The first play of an uncached file can be delayed, so keep files small. The supplied synthesized sounds have no loading delay.
