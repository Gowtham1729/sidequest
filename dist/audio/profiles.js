...
import {recordedLoops} from './recorded.js';
...
export function getAudioProfile(gameId) {
  const profile = audioProfiles[gameId] || extendedProfiles[gameId] || expansionProfiles[gameId] || arcadeProfile;
  const loop = recordedLoops[gameId];
  return loop ? {...profile, music: loop} : profile;
}
