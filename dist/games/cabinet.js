...
export function cabinet(mount,api,{accent='#8ce9d0',height=480}={}){
  ...
  const layout=()=>{const f=s.view.field,k=Math.min(f.w/360,f.h/height);return {k,x:f.x+(f.w-360*k)/2,y:f.y+(f.h-height*k)/2};};
  const q={c,accent,height,get active(){return alive&&active;},get score(){return score;},get elapsed(){return elapsed;},
    local(p){const l=layout();return {x:(p.x-l.x)/l.k,y:(p.y-l.y)/l.k};},
    ...
    finish(title,subtitle,win=false){if(!q.active)return;active=false;api.finish(title,subtitle,score,win?'win':'finish');},
    header(left,right=''){
      const size=Math.max(14,Math.min(20,12/(layout().k*s.view.scale)));
    ...
    frame(dt,update,draw){if(!alive)return;dt=clamp(dt,0,.05);if(q.active&&dt>0){elapsed+=dt;update(dt);noticeTime=Math.max(0,noticeTime-dt);particles=particles.filter(p=>{p.t-=dt;p.x+=p.vx*dt;p.y+=p.vy*dt;p.vy+=100*dt;return p.t>0;});}
      clear(c);const l=layout();c.save();c.translate(l.x,l.y);c.scale(l.k,l.k);draw();
